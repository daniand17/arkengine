...
public:
	...
	unsigned int length() const { return m_string.length(); }

	operator std::string() const { return m_string; }
	char charAt(unsigned int i) const { return m_string.at(i); }
	...
	static ArkString Number(unsigned int number) { return ArkString(std::to_string(number)); }
...
{
public:
	unsigned int size() const { return m_stringList.size(); }
	ArkString join(ArkString delim) const;
	ArkString at(unsigned int index) { return index < m_stringList.size() ? m_stringList[index] : ArkString(); }
